...
SALAD_API_KEY_KRY2=<optional private key for kry2 only, if distinct from kry1>
...
same env var name so the SQLite API budget is shared. For example, `kry2`
...
Per-org worker loops are not included by default because the runtime monitor
already runs all-org workers under one confirmed live action gate. Use
`--include-workers` for read-only worker loops, or `--apply-workers` for live
per-org worker loops. Each tmux session clears stale `PRL_ENABLED_ORGS` and
uses `config/fleet.current.json` by default, so newly added orgs are not hidden
by an old shell filter. The supervisor uses the live `salad-orch-v2-*` tmux
session names, so `--ensure` does not duplicate an already-running stack. If
account cookie jars exist under `state/portal_balance_accounts/`, it starts
`portal_multi_balances.py` instead of the single-account balance watcher.
...
For live TH, it prefers fresh mapped PearlFortune worker rows, then
`slots.live_hashrate_th` from the latest successful guard snapshot, then the
latest per-slot `profit_snapshots` batch when slot rows have not been refreshed
yet.
...
2. Shadow all orgs with live read-only worker observations:
   ...
   pass `--allow-live-retarget` or `--allow-running-nohash-retarget`, so running
   slots remain protected even when the GPU is active before the miner appears
   in the pool.
   ...
   `--pending-retarget-after-seconds` is only allowed to affect running no-hash
   slots when `--allow-running-nohash-retarget` is explicitly passed. Without
   that flag, running no-hash slots are observed and left alone. Creating/allocating/deploying slots use the separate
   `--pending-status-retarget-after-seconds` grace, defaulting to at least 120
   seconds. During aggressive fill with scarce GPU inventory, `900` seconds is
   the current operational target; it gives Salad and Pearl time to allocate,
   pull, start, and report pool hashrate before any rotation. The scheduler's
   ...
   orgs sharing one key are automatically batched apart. The supervisor default
   organization parallelism is 10 and can also be set with
...
availability probe, scheduler, guard, runtime monitor, and active/balance audit.
Per-org worker loops are optional and are only included with `--include-workers`
or `--apply-workers`. It loads the private `.env` file before starting each tmux
session, clears stale `PRL_ENABLED_ORGS`, defaults to `config/fleet.current.json`,
then uses DB heartbeats to avoid restart loops.
...
PRL_WATCH_MIN_PROFIT_USD_DAY=0.30
